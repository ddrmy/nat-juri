// src/pages/codigos/codigo-civil/EstatutoPessoaDeficiencia.tsx
'use client';
import { useGetLegisAll } from '@/api/legis';
import { legisItem } from '@/types/legis';
import { executeEstatutoPessoaDeficiencia } from '@/utils/axios';
...
const EstatutoPessoaDeficiencia: React.FC = () => {
  ...
  useEffect(() => {
    if (legis && legis.length > 0) {
      ...
      const filteredLegis = legis.filter(item => item.titulo === 'estatuto-pessoa-deficiencia');
  ...
  const handleExecutePythonCodigo = async () => {
    try {
      await executeEstatutoPessoaDeficiencia();
      setLegisData(legis);
    } catch (error) {
