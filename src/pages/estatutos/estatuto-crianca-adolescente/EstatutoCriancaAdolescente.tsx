...
import { executeEstatutoCriancaAdolescente } from '@/utils/axios';
import React, { useCallback, useEffect, useRef, useState } from 'react';

const EstatutoCriancaAdolescente: React.FC = () => {
  ...
  useEffect(() => {
    if (legis && legis.length > 0) {
      ...
      const filteredLegis = legis.filter(item => item.titulo === 'estatuto-crianca-adolescente');
  ...
  const handleExecutePythonCodigo = async () => {
    try {
      await executeEstatutoCriancaAdolescente();
      setLegisData(legis);
    } catch (error) {
...
export default EstatutoCriancaAdolescente;
