...
import { executeCodigoProcessoPenal } from '@/utils/axios';
import React, { useCallback, useEffect, useRef, useState } from 'react';

const CodigoProcessoPenal: React.FC = () => {
  ...
  useEffect(() => {
    if (legis && legis.length > 0) {
      ...
      const filteredLegis = legis.filter(item => item.titulo === 'codigo-processo-penal');
  ...
  const handleExecutePythonCodigo = async () => {
    try {
      await executeCodigoProcessoPenal();
      setLegisData(legis);
    } catch (error) {
...
export default CodigoProcessoPenal;
