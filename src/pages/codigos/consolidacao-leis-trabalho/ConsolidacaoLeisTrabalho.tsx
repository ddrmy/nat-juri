...
import { executeConsolidacaoLeisTrabalho } from '@/utils/axios';
import React, { useCallback, useEffect, useRef, useState } from 'react';

const ConsolidacaoLeisTrabalho: React.FC = () => {
  ...
  useEffect(() => {
    if (legis && legis.length > 0) {
      ...
      const filteredLegis = legis.filter(item => item.titulo === 'consolidacao-leis-trabalho');
  ...
  const handleExecutePythonCodigo = async () => {
    try {
      await executeConsolidacaoLeisTrabalho();
      setLegisData(legis);
    } catch (error) {
...
export default ConsolidacaoLeisTrabalho;
