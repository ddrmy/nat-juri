...
import { executeCodigoProcessoCivil } from '@/utils/axios';
import React, { useCallback, useEffect, useRef, useState } from 'react';

const CodigoCivil: React.FC = () => {
  ...
  useEffect(() => {
    if (legis && legis.length > 0) {
      ...
      const filteredLegis = legis.filter(item => item.titulo === 'codigo-processo-civil');
  ...
  const handleExecutePythonCodigo = async () => {
    try {
      await executeCodigoProcessoCivil();
      setLegisData(legis);
    } catch (error) {
